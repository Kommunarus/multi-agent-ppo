...
class Agents:
    ...
    def choose_action(self, obs, last_action, agent_num, evaluate=False):
        inputs = obs.copy()
        ...
        dop_inputs = last_action
        if self.args.reuse_network:
            dop_inputs = np.hstack((dop_inputs, agent_id))

        policy_hidden_state = self.policy.policy_hidden[:, agent_num, :]

        inputs = torch.tensor(inputs, dtype=torch.float32).unsqueeze(0)
        dop_inputs = torch.tensor(dop_inputs, dtype=torch.float32).unsqueeze(0)
        # avail_actions = torch.tensor(avail_actions, dtype=torch.float32).unsqueeze(0)
        if self.args.use_gpu:
            inputs = inputs.cuda()
            dop_inputs = dop_inputs.cuda()
        ...
        policy_q_value, self.policy.policy_hidden[:, agent_num, :] = self.policy.policy_rnn.forward(
            inputs, dop_inputs, policy_hidden_state)
        centr = (5,5)
