...
class Runner:
    def __init__(self, env, args, dict_grid):
        self.env = env
        # env = gym.make("rware-tiny-2ag-v1", msg_bits=1)
    ...
    def evaluate_render(self):
        episode_rewards = 0
        for epoch in range(self.args.evaluate_epoch):
            episode_reward = self.rolloutWorker.save_replay(epoch)
            episode_rewards += episode_reward
