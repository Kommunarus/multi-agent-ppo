...
class IPPO:
    ...
    def _get_actor_inputs(self, batch, transition_idx):
        ...
        if self.args.last_action:
            if transition_idx == 0:
                inputs_dop.append(torch.zeros_like(u_onehot[:, transition_idx]))
            else:
                inputs_dop.append(u_onehot[:, transition_idx - 1])
        ...
        inputs_dop = torch.cat([x.reshape(episode_num * self.args.n_agents, -1) for x in inputs_dop], dim=1)
