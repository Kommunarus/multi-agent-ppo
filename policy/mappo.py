...
class MAPPO:
    def __init__(self, args):
        ...
        critic_input_shape, critic_input_shape_dop = self._get_critic_input_shape()

        # if args.last_action:
        dop_actor_input_shape = self.n_actions
        if args.reuse_network:
            dop_actor_input_shape += 2
        # dop_actor_input_shape += self.n_agents
        self.args = args

        self.policy_rnn = PPOActor(actor_input_shape,dop_actor_input_shape,  args)
        self.eval_critic = PPOCritic(critic_input_shape_dop, self.args)
    ...
    def _get_critic_input_shape(self):
        ...
        # agent_id
        input_shape_dop= 2
        # input_shape_dop= self.n_agents

        # input_shape += self.n_actions * self.n_agents * 2  # 54
        return input_shape, input_shape_dop
    ...
    def _get_critic_inputs(self, batch, transition_idx, max_episode_len):
        ...
        inputs, inputs_next = [], []
        inputs_dop, inputs_next_dop = [], []


        inputs.append(obs)
        inputs_next.append(obs_next)

        # inputs_dop.append(torch.eye(self.n_agents).unsqueeze(0).expand(episode_num, -1, -1))
        # inputs_next_dop.append(torch.eye(self.n_agents).unsqueeze(0).expand(episode_num, -1, -1))
        inputs_dop.append(torch.zeros(2).unsqueeze(0).expand(episode_num*self.n_agents, -1))
        inputs_next_dop.append(torch.zeros(2).unsqueeze(0).expand(episode_num*self.n_agents, -1))

        inputs = torch.cat([x.reshape(episode_num * self.n_agents, x.shape[2], x.shape[3], x.shape[4]) for x in inputs], dim=1)
        inputs_next = torch.cat([x.reshape(episode_num * self.n_agents, x.shape[2], x.shape[3], x.shape[4]) for x in inputs_next], dim=1)

        # inputs_dop = torch.cat([x.reshape(episode_num * self.n_agents, -1) for x in inputs_dop], dim=1)
        # inputs_next_dop = torch.cat([x.reshape(episode_num * self.n_agents, -1) for x in inputs_next_dop], dim=1)
        inputs_dop = torch.cat([x for x in inputs_dop], dim=1)
        inputs_next_dop = torch.cat([x for x in inputs_next_dop], dim=1)

        return inputs, inputs_next, inputs_dop, inputs_next_dop

    def _get_values(self, batch, max_episode_len):
        episode_num = batch['o'].shape[0]
        v_evals, v_targets = [], []
        for transition_idx in range(max_episode_len):
            inputs, inputs_next, inputs_dop, inputs_next_dop = self._get_critic_inputs(batch, transition_idx, max_episode_len)
            if self.args.use_gpu:
                inputs = inputs.cuda()
                inputs_dop = inputs_dop.cuda()
                # inputs_next = inputs_next.cuda()
                self.eval_critic_hidden = self.eval_critic_hidden.cuda()
                # self.target_critic_hidden = self.target_critic_hidden.cuda()

            v_eval, self.eval_critic_hidden = self.eval_critic(inputs, inputs_dop, self.eval_critic_hidden)
    ...
    def _get_actor_inputs(self, batch, transition_idx):
        ...
        if self.args.last_action:
            if transition_idx == 0:
                inputs_dop.append(torch.zeros_like(u_onehot[:, transition_idx]).reshape(episode_num * self.args.n_agents, -1))
            else:
                inputs_dop.append(u_onehot[:, transition_idx - 1].reshape(episode_num * self.args.n_agents, -1))
        if self.args.reuse_network:
            inputs_dop.append(torch.zeros(2).unsqueeze(0).expand(episode_num * self.n_agents, -1))
            # inputs_dop.append(torch.eye(self.args.n_agents).unsqueeze(0).expand(episode_num, -1, -1))

        inputs = torch.cat([x.reshape(episode_num * self.args.n_agents, x.shape[2], x.shape[3], x.shape[4]) for x in inputs], dim=1)
        inputs_dop = torch.cat([x for x in inputs_dop], dim=1)

        return inputs, inputs_dop
    ...
    def _get_action_prob(self, batch, max_episode_len):
        ...
        action_prob = []
        for transition_idx in range(max_episode_len):
            inputs, input_dop = self._get_actor_inputs(batch, transition_idx)
            if self.args.use_gpu:
                inputs = inputs.cuda()
                input_dop = input_dop.cuda()
                self.policy_hidden = self.policy_hidden.cuda()
            outputs, self.policy_hidden = self.policy_rnn(inputs, input_dop, self.policy_hidden)
    ...
    def load_model(self, num):
        self.eval_critic.load_state_dict(torch.load(self.model_dir + '/' + num + '_critic_params.pkl', map_location=torch.device('cpu')))
        self.policy_rnn.load_state_dict(torch.load(self.model_dir + '/' + num + '_rnn_params.pkl', map_location=torch.device('cpu')))
